...
def methodName = 'SortByMsgDescMethod'
...
def messages = ['4msg/toSort','/message5toSort','6msg/3toSort']
...
def sortByMsgDesc = "?Sorts=-message"

// act
def sortByMsgDescResponse = WS.sendRequestAndVerify(findTestObject('Background Task Executions Logs/Get Background Task Executions Logs', [('queryString') : sortByMsgDesc]))

//assert
def logsCount = WS.getElementsCount(sortByMsgDescResponse, '')

def boolean isNotSorted = false

for (def i=0;i<logsCount-1;i++){
	if(WS.getElementPropertyValue(sortByMsgDescResponse, "[${i}].message")<WS.getElementPropertyValue(sortByMsgDescResponse,"[${i+1}].message")){
		isNotSorted = true
		break
	}
}
