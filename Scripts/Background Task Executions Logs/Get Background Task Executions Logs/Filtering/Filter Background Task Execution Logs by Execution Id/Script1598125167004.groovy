...
def methodName = 'FilterByIdMethod'
...
def filterLogsByExecId = "?Filters=backgroundTaskExecutionId==$executionId"

// act
def filterByExecIdResponse = WS.sendRequestAndVerify(findTestObject('Background Task Executions Logs/Get Background Task Executions Logs', [('queryString') : filterLogsByExecId]))

//assert
WS.verifyElementsCount(filterByExecIdResponse, '',1)

WS.verifyElementPropertyValue(filterByExecIdResponse, '[0].id', logId)

WS.verifyElementPropertyValue(filterByExecIdResponse, '[0].backgroundTaskExecutionId', executionId)
