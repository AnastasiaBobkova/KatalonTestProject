...
def methodName = 'FilterByNotEqlMsgMethod'
...
def messages = ['msg3*withspecsymbols','msg4*withspecsymbols/']
...
def messageToFilter=messages[0]
def filterLogsByNotEqlMsg = "?Filters=message!=$messageToFilter"

// act
def filterByNotEqlMsgResponse = WS.sendRequestAndVerify(findTestObject('Background Task Executions Logs/Get Background Task Executions Logs', [('queryString') : filterLogsByNotEqlMsg]))

//assert
def logsCount = WS.getElementsCount(filterByNotEqlMsgResponse, '')

def boolean isLogFound = false
for (def i=0;i<logsCount;i++){
		if (WS.getElementPropertyValue(filterByNotEqlMsgResponse, "[${i}].message")==messageToFilter){
		isLogFound=true
		break
	}
}
