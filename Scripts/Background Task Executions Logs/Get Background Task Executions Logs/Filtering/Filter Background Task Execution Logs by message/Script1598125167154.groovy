...
def methodName = 'FilterByMsgMethod'
...
def messages = ['msg1withspecsymbols/','msg2withspecsymbols/']
...
def filterLogsByMsg = "?Filters=message==$messageToFilter"

// act
def filterByMsgResponse = WS.sendRequestAndVerify(findTestObject('Background Task Executions Logs/Get Background Task Executions Logs', [('queryString') : filterLogsByMsg]))

//assert
def logsCount = WS.getElementsCount(filterByMsgResponse, '')

def boolean isLogFound = true
for (def i=0;i<logsCount;i++){
		if (WS.getElementPropertyValue(filterByMsgResponse, "[${i}].message")!=messageToFilter){
		isLogFound=false
		break
	}
}
