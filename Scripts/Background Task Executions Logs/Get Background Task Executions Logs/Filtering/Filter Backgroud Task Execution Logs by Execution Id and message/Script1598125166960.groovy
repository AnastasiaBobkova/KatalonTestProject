...
def methodName = 'FilterByExecIdAndMsgMethod'
...
def messageToFilter = "msg1/To'Filter"

WS.sendRequestAndVerify(findTestObject('Background Task Executions Logs/Create Background Task Executions Log', [('id') : "\"${logId}\""
	, ('executionId') : "\"${executionId}\"", ('message') : "\"${messageToFilter}\""]))


def filterLogsByExecIdMsg = "?Filters=backgroundTaskExecutionId==$executionId,message==$messageToFilter"

// act
def filterByExecIdMsgResponse = WS.sendRequestAndVerify(findTestObject('Background Task Executions Logs/Get Background Task Executions Logs', [('queryString') : filterLogsByExecIdMsg]))

//assert
WS.verifyElementsCount(filterByExecIdMsgResponse, '',1)

WS.verifyElementPropertyValue(filterByExecIdMsgResponse, '[0].backgroundTaskExecutionId', executionId)

WS.verifyElementPropertyValue(filterByExecIdMsgResponse, '[0].message', messageToFilter)
