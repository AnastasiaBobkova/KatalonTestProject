...
def logIds = [UUID.randomUUID().toString(),UUID.randomUUID().toString()]
...
def filterLogsById = "?Filters=id==$logIdToFilter"

// act
def filterByLogIdResponse = WS.sendRequestAndVerify(findTestObject('Background Task Executions Logs/Get Background Task Executions Logs', [('queryString') : filterLogsById]))

//assert
WS.verifyElementsCount(filterByLogIdResponse, '', 1)

WS.verifyElementPropertyValue(filterByLogIdResponse, '[0].id', logIdToFilter)
