...
def methodName = 'MethodForNotUniqueLogId'
...
def createLogResponse = WS.sendRequestAndVerify(findTestObject('Background Task Executions Logs/Create Background Task Executions Log', [('id') : "\"${logId}\""
	, ('executionId') : "\"${executionId}\"", ('message') : "\"${message}\""]))

// act
def createDuplicateLogResponse = WS.sendRequest(findTestObject('Background Task Executions Logs/Create Background Task Executions Log', [('id') : "\"${logId}\""
	, ('executionId') : "\"${executionId}\"", ('message') : "\"${message}\""]))

//assert
WS.verifyResponseStatusCode(createDuplicateLogResponse, 400)
